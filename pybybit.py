...
# ---------- Utility: query Bybit server time and print drift ----------
def get_bybit_server_time(demo=True):
    """Returns tuple (server_ts_ms, local_ts_ms, drift_ms)."""
    base = "https://api-demo.bybit.com" if demo else "https://api.bybit.com"
    endpoint = base + "/v5/public/time"
    try:
        ...
        if isinstance(j, dict):
            # common shapes: {"time": 1759...} or {"time_now": "..."} or {"result": {"time": ...}}
            if "time" in j:
                server_ts = int(j["time"])
            elif "time_now" in j:
                # sometimes time_now is a string of seconds (e.g. "169..."), convert to ms
                tn = j["time_now"]
                try:
                    # if fractional seconds present, make ms
                    server_ts = int(float(tn) * 1000)
                except Exception:
                    server_ts = int(tn)
            elif "result" in j and isinstance(j["result"], dict):
                res = j["result"]
                if "time" in res:
                    server_ts = int(res["time"])                
        if server_ts is None:
            # fallback: try to find any integer-looking value in json
            for v in j.values() if isinstance(j, dict) else []:
                try:
                    ...
                    continue
        if server_ts is None:
            raise ValueError("Could not parse server time from Bybit response")
        # normalize: if server_ts looks like seconds (10 digits) convert to ms
        if server_ts < 10**12:
            server_ts = int(server_ts * 1000)
        local_ts = int(time.time() * 1000)
        drift = local_ts - server_ts
        return server_ts, local_ts, drift
    except Exception as e:
        print(f"[WARN] Could not fetch Bybit server time: {e}")
        return None, int(time.time() * 1000), None
...
def trade_tcl(keys_dict, order_dict, tpsl_dict, demo=True, max_wait_seconds=300):
    ...
    # quick check: print drift vs server time
    server_ts, local_ts, drift = get_bybit_server_time(demo=demo)
    if drift is not None:
        ...
        if abs(drift) > RECV_WINDOW_MS:
            print(f"[WARN] Absolute drift ({abs(drift)} ms) exceeds configured recv_window ({RECV_WINDOW_MS} ms). "
                  "Requests may still be rejected until you correct system clock or increase recv_window.")
    else:
        print("[INFO] Could not determine Bybit server time; proceeding with configured recv_window.")
    ...
    def place_orders(account_name, creds):
        ...
        # set recv_window to the configured value
        try:
            session = HTTP(api_key=creds["api_key"], api_secret=creds["api_secret"], demo=demo, recv_window=RECV_WINDOW_MS)
        except TypeError:
            # older pybit variants may use testnet instead of demo; try fallback
