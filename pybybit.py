...
def use_bybit_server_time_patch(demo=True, verbose=True, fallback_ok=False):
    """Context manager that patches time.time() and time.time_ns() to Bybit server time.

    Yields True if patched, False otherwise. Restores originals on exit.
    """
...
def trade_tcl(keys_dict, order_dict, tpsl_dict, demo=True, max_wait_seconds=300):
    ...
    server_ts, local_ts, drift = None, None, None
    try:
        server_ts, local_ts, drift = _fetch_bybit_server_time_ms(demo=demo) and (_fetch_bybit_server_time_ms(demo=demo), int(time.time() * 1000), int(time.time() * 1000) - _fetch_bybit_server_time_ms(demo=demo))
    except Exception:
        server_ts, local_ts, drift = None, int(time.time() * 1000), None
    ...
    # run placement for all accounts in parallel (use server-time patch while placing)
